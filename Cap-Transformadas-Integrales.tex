...
En algunas ocasiones, nos interesará trabajar con funciones $f(t)$ para las cuales la transformada de Fourier no existe, pues la función no se anulará cuando $t \to \infty$, de modo que la integral \eqref{T.Fourier} no convergerá. Por ejemplo, la función $f(t) = t$ no posee transformada de Fourier. Es más, podríamos estar interesados en el comportamiento de ciertas funciones únicamente cuando $t > 0$, como podría ser el caso en problemas de valores iniciales. Para estos casos, utilizamos la \textbf{transformada de Laplace}.
...
Para asegurar la existencia de una transformada de Laplace, primero debemos introducir la siguiente definición.

% A partir de la definición, es claro que $f$ debe escogerse de manera que
% \begin{equation}
% \int_0^{t_0} e^{-st} f(t) dt \label{Laplace1}
% \end{equation}
% exista para todo $t_0 > 0$. Esto se logra exigiendo que $f$ sea seccionalmente continua en todo el intervalo $[0,t_0]$, con $t_0>0$. No obstante, no es suficiente para garantizar la existencia de $\mathcal{L}\{f(t)\}$, ya que al tomar el límite $t_0 \to \infty$ de \eqref{Laplace1}, esta debe converger. A grandes rasgos, mostraremos que la transformada de Laplace de una función continua por partes existe, siempre que la función no crezca más rápido que una exponencial.
...
\begin{teorema}[\textbf{de existencia}] \label{ExistenciaLaplace}
...
\begin{obs}{Observación}
    Existen funciones que tienen transformada de Laplace y que no satisfacen las hipótesis del teorema. Por ejemplo, $f(t) = \frac{1}{\sqrt{t}}$ no es de orden exponencial, pues tiende a infinito cuando $t \to 0^+$. Sin embargo,
    \begin{equation*}
...
\begin{teorema}
    Bajo las mismas condiciones del teorema \ref{ExistenciaLaplace}, la transformada de Laplace $F(s)$ es analítica en $\real(s) \geq \alpha_1 > \alpha$, es decir, la derivada $F'(s)$ existe en dicho semiplano.
\end{teorema}

\begin{propo}
    Podemos definir la derivada de una transformada de Laplace como 
    \begin{equation}
        \frac{d^n}{ds^n} F(s) = (-1)^n \mathcal{L}\{t^n f(t)\} \, ,
    \end{equation}
    función que, en general, existe.
\end{propo}
% En general, existe
% \begin{shaded}
% $$$$
...
\begin{corolario}
    Si $\lim\limits_{s \to + \infty} \mathcal{L}\{f(t)\} \neq 0$, entonces no existe $f(t)$ tal que $F = \mathcal{L}\{f(t)\}$.
\end{corolario}

\begin{obs}{Observación}
    Como consecuencia del corolario anterior, podemos decir inmediatamente que funciones tales como $\frac{s+4}{s-1}, s, s^2, \sin(s)$, etc. no tienen transformada inversa de Laplace.
\end{obs}
...
La primera pregunta que podemos hacernos es cómo asegurar la existencia de la transformada inversa. La respuesta no es sencilla, mas sí podemos asegurar que \textbf{cualquier función que no converge a cero cuando} $s \to \infty$, \textbf{no posee transformada de Laplace inversa}.
...
\begin{obs}{Observación}
    Siendo rigurosos, se debería escribir 
    \begin{equation*}
        f(t) = \frac{1}{2\pi i} \text{ V. P.} \int_{\gamma - i \infty}^{\gamma + i \infty} e^{st} F(s) \,ds \, ,
    \end{equation*}
    pues
    \begin{equation*}
...
\begin{teorema}
   ...
   Entonces, la transformada de Laplace inversa de $F(s)$ es
    $$f(t) = \mathcal{L}^{-1}\{F(s)\} = \sum_{k= 1}^n \operatorname{Res}(e^{st} F(s), s_k), \quad t > 0.$$
\end{teorema}
...
\begin{ejemplo}
    ...
    \begin{align*}
        \operatorname{Res}(\psi, 1) &= \lim_{s \to 1} (s-1) \psi(s) = \lim_{s\to 1} \left[e^{st} \frac{s^2}{(s+1)(s-i)(s+i)} \right] = \frac{e^t}{4},\\
        \operatorname{Res}(\psi, -1) &= \lim_{s \to -1} (s+1) \psi(s) = \lim_{s\to -1} \left[e^{st} \frac{s^2}{(s-1)(s-i)(s+i)} \right] = -\frac{e^{-t}}{4}, \\
        \operatorname{Res}(\psi, i) &= \lim_{s \to i} (s-i) \psi(s) = \lim_{s\to i} \left[e^{st} \frac{s^2}{(s-1)(s+1)(s+i)} \right] = \frac{e^{it}}{4i}, \\
        \operatorname{Res}(\psi, -i) &= \lim_{s \to -i} (s+i) \psi(s) = \lim_{s\to -i} \left[e^{st} \frac{s^2}{(s-1)(s+1)(s-i)} \right] = -\frac{e^{-it}}{4i}.
    \end{align*}

    Por lo tanto,
    \begin{align*}
        \mathcal{L}^{-1} \left\{\frac{s^2}{s^4-1} \right\} &= \operatorname{Res}(\psi,1) + \operatorname{Res}(\psi,-1) + \operatorname{Res}(\psi,i) + \operatorname{Res}(\psi, -i) \\
...
\begin{enumerate}
    ...
    \item \textbf{Derivada de una transformada de Laplace:} Para $\real(s) > \alpha$, se cumple que
    \begin{equation*}
        \dfrac{d^n}{ds^n} \left[ \mathcal{L}\{f\}(s) \right] = (-1)^n \mathcal{L}\{ t^n f(t) \}(s) \ .
    \end{equation*}

    \item \textbf{Transformada de una fracción:} Dada una función de la forma $f(t)/t$, tal que $\lim\limits_{t \to 0} f(t)/t$ existe, su transformada de Laplace puede obtenerse como 
    \begin{equation*}
...
Al igual que para la transformada de Fourier, la transformada de Laplace de la convolución obedece el siguiente teorema, donde ahora la integral se realiza sobre el intervalo $(0, t)$:
\begin{equation*}
    (f*g)(t) = \int_0^t f(u) g(t-u) \, du \ . 
\end{equation*}

\begin{teorema}[\textbf{de Convolución de Laplace}] \label{Teo:Convolucion}
...
Buscamos reducir una ecuación diferencial ordinaria en una ecuación algebraica, para lo cual utilizaremos la transformada que sea más conveniente para cada caso.
...
A diferencia de la transformada de Fourier, utilizar la transformada de Laplace para resolver una ecuación diferencial ordinaria requiere conocer su valor en algún punto dado. Esto es de especial utilidad para trabajar problemas de valores iniciales para cualquier función a la que no podamos calcular su transformada de Fourier.
...
\begin{ejemplo}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=8cm]{Figuras/Circuito.pdf}
...
Utilizando la transformada de Fourier, podemos (potencialmente) reducir una ecuación diferencial parcial en dos variables, a una ecuación diferencial ordinaria en una variable en el espacio de Fourier, que será en principio más fácil de resolver. Luego, aplicando la transformada inversa, podemos recuperar la solución a la EDP original.

¿En qué momento esto es útil? Supongamos que tenemos una función de dos variables, por ejemplo, de la posición $x$ y del tiempo $t$. Al calcular la transformada de Fourier, lo hacemos \textbf{respecto de una sola variable}, ya sea de la posición, gracias a la cual pasamos al dominio del número de onda, o respecto del tiempo, caso en el que pasamos al dominio de las frecuencias. Por ello, si tenemos una ecuación diferencial que involucra tanto derivadas respecto a la posición como derivadas respecto al tiempo, podemos aplicar la transformada de Fourier respecto de una de las variables, tras lo cual la segunda actuará como una ``constante'' en el sistema.
...
El proceso a realizar puede sintetizarse en la figura \ref{fig:Diagrama-Fourier}.
\begin{figure}[htbp]
    \centering
