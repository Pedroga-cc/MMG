...
\begin{itemize}
    \item \textbf{Adición y sustracción.} Se define la adición (o suma) y sustracción (o resta) de dos tensores \emph{del mismo rango} componente a componente, es decir,
    \begin{align}
    ...
    \item \textbf{Contracción de índices.} El producto escalar entre dos vectores nos entrega un escalar en lugar de un tensor de rango 2, como consecuencia de la repetición de índices en el producto. De manera similar, la repetición de dos índices dentro de un tensor de rango $r$ (digamos, el $s$-ésimo y el $t$-ésimo índice), es equivalente a un tensor de rango $r-2$,
    \begin{equation}
    ...
    \end{equation}
    donde sabemos que $B$ y $C$ son tensores de rango $r$ y $s$, respectivamente, pero desconocemos si $A$ es un tensor. La ley del cociente establece que \emph{si la relación \eqref{eq:ley-cociente} es válida en cualquier sistema de coordenadas, entonces} $A$ es un tensor de rango $s-r+2$. La demostración (para el caso $r=s=2$) puede ser hallada en el capítulo 26, sección 7 de Riley \cite{Riley}.
...
Hasta ahora, de manera implícita, hemos utilizado transformaciones ortogonales \emph{propias}, es decir, que solo rotan el sistema, pero no modifican la orientación de los tensores. Sin embargo, al considerar transformaciones \emph{impropias}, que no solo rotan el sistema sino que realizan una inversión de coordenadas o \emph{reflexión} ($x_i \to -x_i$). Cuando también incluimos este tipo de transformaciones, los vectores siguen cumpliendo la regla de transformación \eqref{eq:transformacion-vector}. En cambio, existen ciertas cantidades físicas que comúnmente supondríamos como vectores, que \textbf{no transforman de igual manera} bajo una reflexión, por ejemplo, aquellas relacionadas a cantidades \emph{angulares}, como la velociadad angular, el torque o el momento angular.
...
\begin{defi} \marginnote{Símbolo de Levi-Civita}
...
\begin{propiedad}
    ...
    \begin{enumerate}
        ...
        \begin{equation}
            det(A) = a_{1 j_1} a_{2 j_2} \dots a_{i_n j_n} \varepsilon_{j_1 j_2 \dots j_n} \ .
        \end{equation}
...
En el capítulo anterior, estudiamos la noción de \emph{análisis vectorial}, que correspondía al uso del operador nabla en diferentes sistemas coordenados. En este, hicimos uso de las nociones de \emph{campo escalar} y \emph{campo vectorial}. Estas nociones pueden también extenderse a elementos de mayor rango mediante los campos tensoriales.
...
\end{align}
comprobando así que transforma como un tensor de rango $r+1$. Esta idea es muchas veces entendida como el \emph{gradiente del tensor} $T$, $\nabla T$. Además, hemos usado que $\dfrac{\partial x_k}{\partial x_j'} = a_{jk}$, lo que revisaremos brevemente en la siguiente sección.

También podemos calcular la derivada de un tensor de rango $r$ respecto de un parámetro $t$ \emph{independiente de las coordenadas}. En este caso, el resultado es también un tensor de rango $r$,
\begin{equation}
    \frac{d T'_{i_1 \dots i_r}}{dt} = a_{i_1 j_1} \dots a_{i_r j_r} \frac{dT_{j_1 \dots j_r}}{dt} \ . 
\end{equation}
...
\end{equation*}
Típicamente, se habla de \emph{divergencia} cuando la derivación ocurre sobre el primer índice del tensor, pero es válido definirla para cualquier otro índice, teniendo el cuidado de ser explícitos sobre cuál índice realizaremos la contracción.
